...
const _obInstitutionSbcreateImgNode = ({
    url: url,
    className: className,
    alt: alt
}) => {
...
const _createInstitutionContainer = () => {
    ...
    obInstitutionSbModalContent.className = "institution-modal-content";
    obInstitutionSbModalContent.id = "institution-modal-content";
    const institutionContainer = _obInstitutionSbcreateHTMLNode("div", "institution-container", obInstitutionSbModalContent);
...
function _createInstitutionBankListView(body, institutionLogos, config) {
    if (config.countryFilter) _addBackArrow({
        visible: true
    });
    ...
    institutionLogos.forEach((el => {
        ...
        institutionRow.href = 'institute/'+el.id;
...
const _filterByCountry = (institutions, country) => institutions.filter((inst => inst.countries.includes(country)));
